...
char chunk3[100000];
char chunk4[100000];
char list1[10000], list2[10000],list3[10000],list4[10000];
...
void *connection_handler(void *server_number){
    ...
    if (strcasecmp(command, "put") ==0){
        ...
        switch(first_chunk){
            case 0:
                pthread_mutex_lock(&mutex1);
                memcpy(chunk1,buffer,100000);
                pthread_mutex_unlock(&mutex1);
                break;
            case 1:
                pthread_mutex_lock(&mutex2);
                memcpy(chunk2,buffer,100000);
                pthread_mutex_unlock(&mutex2);
                break;
            case 2:
                pthread_mutex_lock(&mutex3);
                memcpy(chunk3,buffer,100000);
                pthread_mutex_unlock(&mutex3);
                break;
            case 3:
                pthread_mutex_lock(&mutex4);
                memcpy(chunk4,buffer,100000);
        ...
        switch(second_chunk){
            case 0:
                pthread_mutex_lock(&mutex1);
                memcpy(chunk1,buffer2,100000);
                pthread_mutex_unlock(&mutex1);
                break;
            case 1:
                pthread_mutex_lock(&mutex2);
                memcpy(chunk2,buffer2,100000);
                pthread_mutex_unlock(&mutex2);
                break;
            case 2:
                pthread_mutex_lock(&mutex3);
                memcpy(chunk3,buffer2,100000);
                pthread_mutex_unlock(&mutex3);
                break;
            case 3:
                pthread_mutex_lock(&mutex4);
                memcpy(chunk4,buffer2,100000);
                pthread_mutex_unlock(&mutex4);
                break;
        }
    }
        ...
        switch(number){
            case 1:
                // Receive the 1st message
                if ((bytes_read = recv(socket_desc, list1, sizeof(list1), 0)) < 0) {
                    perror("recv");
                }
                break;
            case 2:
                // Receive the 1st message
                if ((bytes_read = recv(socket_desc, list2, sizeof(list2), 0)) < 0) {
                    perror("recv");
                }
                break;
            case 3:
                // Receive the 1st message
                if ((bytes_read = recv(socket_desc, list3, sizeof(list3), 0)) < 0) {
                    perror("recv");
                }
                break;
            case 4:
                // Receive the 1st message
                if ((bytes_read = recv(socket_desc, list4, sizeof(list4), 0)) < 0) {
                    perror("recv");
                }
                break;
        }
...
if (pthread_join(dfs4, NULL) != 0) {
    ...
    fwrite(chunk1, 1, size/4, output);

    // Write chunk2 to the output file
    fwrite(chunk2, 1, size/4, output);

    // Write chunk2 to the output file
    fwrite(chunk3, 1, size/4, output);

    // Write chunk2 to the output file
    fwrite(chunk4, 1, (size/4)+(size%4), output);
